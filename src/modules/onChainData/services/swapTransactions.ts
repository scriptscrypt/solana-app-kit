        ...
        try {
          ...
          if (txData.result && 
            ...
            if (uniqueMints.size >= 2) {
              ...
              const parsedSwap = parseSwapFromTransaction(txData.result, swapTx);
              
              // Only include transactions where both input and output tokens were identified
              if (parsedSwap.inputToken.mint && parsedSwap.outputToken.mint && 
...
export const enrichSwapTransactions = async (swaps: SwapTransaction[]): Promise<SwapTransaction[]> => {
  try {
    ...
    swaps.forEach(swap => {
      if (swap.inputToken.mint && swap.inputToken.mint !== 'Unknown') tokenMints.add(swap.inputToken.mint);
      if (swap.outputToken.mint && swap.outputToken.mint !== 'Unknown') tokenMints.add(swap.outputToken.mint);
    });
    ...
    console.log(`Successfully fetched metadata for ${tokenMetadataMap.size} tokens`);
      ...
      try {
        ...
        if (enrichedSwap.inputToken.mint && tokenMetadataMap.has(enrichedSwap.inputToken.mint)) {
          ...
          enrichedSwap.inputToken = {
            ...
            image: metadata.logoURI || metadata.image || enrichedSwap.inputToken.image,
          };
        }
        ...
        if (enrichedSwap.outputToken.mint && tokenMetadataMap.has(enrichedSwap.outputToken.mint)) {
          ...
          enrichedSwap.outputToken = {
            ...
            image: metadata.logoURI || metadata.image || enrichedSwap.outputToken.image,
          };
        }
...
function parseSwapFromTransaction(
  ...
  try {
    ...
    const preBalanceMints = new Set(userPreBalances.map(balance => balance.mint));
    const newMints = userPostBalances
      .filter(balance => !preBalanceMints.has(balance.mint) && balance.amount > 0)
    ...
    newMints.forEach(mint => {
      const newBalance = userPostBalances.find(balance => balance.mint === mint);
      if (newBalance) {
        increasedTokens.push({
    ...
    const preAccountSOLBalance = txData.meta.preBalances && txData.meta.preBalances[0];
    const postAccountSOLBalance = txData.meta.postBalances && txData.meta.postBalances[0];
    
    // This check helps with swaps that end by unwrapping SOL
    if (preAccountSOLBalance !== undefined && postAccountSOLBalance !== undefined) {
      const preSolAmount = preAccountSOLBalance;
      const postSolAmount = postAccountSOLBalance;
      const solDifference = (postSolAmount - preSolAmount) - (txData.meta.fee || 0); // Adjust for tx fee
      
      if (solDifference > 1000000) { // A significant SOL increase (>0.001 SOL)
      ...
      } else if (solDifference < -1000000) { // A significant SOL decrease (> 0.001 SOL)
        // Native SOL was likely spent (beyond just fees)
        const adjustedDifference = Math.abs(solDifference) + (txData.meta.fee || 0);
    ...
    // Debug log
    console.log(`Swap identified: ${finalInput.mint.substring(0, 6)}... (${finalInput.amount}) → ${finalOutput.mint.substring(0, 6)}... (${finalOutput.amount})`);
