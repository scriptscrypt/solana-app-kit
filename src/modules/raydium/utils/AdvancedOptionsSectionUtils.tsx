...
export function calculateGraphData(tokenSupply: string, solRaised: string, bondingCurve: string) {
  ...
  if (
    ...
    return {
      linePath: `M${defaultX},${defaultY}L${
        defaultX + graphWidth
      },${defaultY}`, // Flat line at bottom
      areaPath: `M${defaultX},${defaultY}L${
        defaultX + graphWidth
      },${defaultY}Z`, // No area
  ...
  const scaleFactor = Math.pow(
    10,
    Math.max(0, Math.floor(Math.log10(maxSupplyOnCurve)) - 6),
  );
  ...
  for (let i = 0; i <= numPoints; i++) {
    ...
    const boundedY = Math.max(
      margin.top,
      Math.min(height - margin.bottom, svgY),
    );
  ...
  const areaPath = `${linePath} L${points[points.length - 1].cx.toFixed(1)},${
